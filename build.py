#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""e.Amber お問い合わせフォームプラグイン ビルドスクリプト。

使い方:
    py build.py 1.0.1 "・○○を修正"

やること:
  1. eamber-form/eamber-form.php の Version ヘッダーと EAF_VER 定数を更新
  2. update.json の version と changelog を更新
  3. eamber-form.zip を再生成（/区切り＝WPで正しく解凍される）
...
PLUGIN_DIR = BASE / "eamber-form"
MAIN_PHP = PLUGIN_DIR / "eamber-form.php"
UPDATE_JSON = BASE / "update.json"
ZIP_PATH = BASE / "eamber-form.zip"

VERSION_RE = re.compile(r"^(\s*\*\s*Version:\s*)(\S+)", re.MULTILINE)
FHSVER_RE = re.compile(r"(define\('EAF_VER',\s*')([^']+)('\))")
...
def bump(version: str, changelog: str):
    ...
    if not FHSVER_RE.search(php):
        sys.exit("エラー: EAF_VER 定数が見つかりません")
    ...
    print(f"[php] Version / EAF_VER -> {version}")
...
def build_zip():
    ...
    with zipfile.ZipFile(ZIP_PATH, "w", zipfile.ZIP_DEFLATED) as z:
        for f in files:
            # アーカイブ内パスは常に "eamber-form/..."（/区切り）
            arc = "eamber-form/" + f.relative_to(PLUGIN_DIR).as_posix()
            z.write(f, arc)
